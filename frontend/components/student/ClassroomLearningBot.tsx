...
import { BookOpen, Bot, FileText, MessageCircle, Send, UserRound } from 'lucide-react';
import { supabase } from '../../database/supabase';
import { useAuth } from '../../context/AuthContext';
import { getApiUrl } from '../../config/apiConfig';

type ChatMessage = { role: 'user' | 'assistant'; content: string; citations?: Array<{ source: string; reason: string }> };
  ...
  const askQuestion = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl('/api/ai/classroom-answer'), {
        ...
        body: JSON.stringify({ classroomId, threadId: threadId || undefined, question: currentQuestion }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || 'The classroom assistant could not answer.');
      setThreadId(data.threadId || '');
      setSources(data.sources || []);
      setMessages((previous) => [...previous, { role: 'assistant', content: data.answer?.answer || 'No grounded answer was returned.', citations: data.answer?.citations || [] }]);
    } catch (err: any) {
  ...
  return (
    <div className="mx-auto flex min-h-full max-w-6xl flex-col gap-6 p-8">
      <header className="flex items-start justify-between gap-4">
        ...
        <select value={classroomId} onChange={(event) => changeClassroom(event.target.value)} disabled={!classrooms.length} className="min-w-[220px] rounded-xl border border-slate-200 bg-white p-3 text-sm font-bold dark:border-white/10 dark:bg-white/5"><option value="">Select classroom</option>{classrooms.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
      </header>
