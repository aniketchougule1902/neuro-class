...
import { Clock, ArrowRight, ArrowLeft, CheckCircle2, AlertCircle, FileText } from 'lucide-react';
...
export const ExamTaker: React.FC<ExamTakerProps> = ({ testId, onExit }) => {
  ...
  const fetchTest = async () => {
    try {
      ...
      // Get the corresponding student profile for this classroom
  ...
  if (loading) {
    return <div className="fixed inset-0 z-[100] bg-slate-50 dark:bg-black flex flex-col items-center justify-center">
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-slate-50 dark:bg-black flex items-center justify-center p-4">
        <div className="bg-white dark:bg-white/5 border border-rose-500/20 p-8 rounded-3xl max-w-md w-full text-center space-y-4">
          ...
          <button onClick={onExit} className="px-6 py-3 rounded-full bg-slate-100 dark:bg-white/10 text-xs font-bold uppercase tracking-widest text-slate-900 dark:text-white">Go Back</button>
        </div>
      </div>
    );
  }
  ...
  if (isSubmitted) {
    return (
      <div className="fixed inset-0 z-[100] bg-slate-50 dark:bg-black flex items-center justify-center p-4">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-white/5 border border-emerald-500/20 p-10 rounded-[40px] max-w-md w-full text-center space-y-6 shadow-2xl">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">Your answers have been securely recorded. You can view your results in the portal once graded.</p>
          </div>
          <button onClick={onExit} className="w-full py-4 rounded-2xl bg-emerald-500 text-white font-bold uppercase tracking-widest text-[11px] shadow-lg shadow-emerald-500/30">
            Return to Dashboard
          </button>
        </motion.div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-50 dark:bg-[#0a0a0a] flex flex-col font-sans">
      ...
      <header className="h-20 bg-white/50 dark:bg-black/50 backdrop-blur-xl border-b border-black/5 dark:border-white/10 flex items-center justify-between px-8 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Question {currentQ + 1} of {test.questions.length}</p>
          </div>
        </div>

        <div className={cn(
          "flex items-center gap-3 px-5 py-2.5 rounded-full border border-black/5 dark:border-white/10 shadow-sm",
          timeLeft < 300 ? "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20 animate-pulse" : "bg-white dark:bg-white/5 text-slate-900 dark:text-white"
        )}>
          <Clock size={16} />
                    ...
                    return (
                      ...
                      >
                        <div className={cn(
                          ...
                          isSelected ? "border-purple-500 bg-purple-500 text-white" : "border-slate-300 dark:border-white/20 group-hover:border-purple-500/50"
                        )}>
      ...
      {/* Footer Navigation */}
      <footer className="h-24 bg-white dark:bg-black border-t border-black/5 dark:border-white/10 flex items-center justify-between px-8 shrink-0">
        <button 
        ...
        <div className="flex gap-2">
          {test.questions.map((_: any, idx: number) => (
            <div key={idx} className={cn(
              ...
              currentQ === idx ? "w-6 bg-purple-500" : (answers[test.questions[idx].id] ? "bg-purple-500/50" : "bg-slate-200 dark:bg-white/10")
            )} />
          ))}
        </div>
