...
export async function uploadClassroomMaterial(request: Request): Promise<Response> {
  try {
    ...
    if (!classroomId || !(file instanceof File)) return json({ error: 'classroomId and file are required.' }, 400);
    const classroom = await ownedClassroom(classroomId, user.id);
    const extension = extensionFor(file.name);
    if (!ALLOWED_MIME.has(file.type) || !EXTENSIONS.has(extension)) return json({ error: 'Unsupported material type. Upload PDF, DOCX, TXT, Markdown, CSV, or JSON.' }, 415);
    if (file.size <= 0 || file.size > MAX_BYTES) return json({ error: 'Material must be between 1 byte and 15 MB.' }, 413);
    ...
    if (uploadError) return json({ error: `Private storage upload failed: ${uploadError.message}` }, 502);
    ...
    if (insertError || !material) {
      ...
      return json({ error: 'Material metadata could not be saved.' }, 500);
    }
    ...
    return json({ material, message: extractionStatus === 'ready' ? 'Material processed and available to the classroom tutor.' : 'Material uploaded. A background worker must extract its text before the tutor can use it.' });
  } catch (error: any) {
    return json({ error: error.message || 'Unable to upload classroom material.' }, error.status || 500);
  }
}
...
export async function listClassroomMaterials(request: Request): Promise<Response> {
  try {
    ...
    if (error) return json({ error: 'Unable to load classroom materials.' }, 500);
    return json({ materials: data || [] });
  } catch (error: any) {
    return json({ error: error.message || 'Unable to load classroom materials.' }, error.status || 500);
  }
}
